...
<img src="public/ziv-logo.png" alt="Ziv Logo" width="140" style="border-radius: 28px; margin-bottom: 16px; box-shadow: 0 12px 36px rgba(225, 29, 72, 0.25);" />
...
[![Gemini 2.5 Flash](https://img.shields.io/badge/Gemini_2.5_Flash-Sub--400ms_Voice-E11D48?style=flat-square&logo=google&logoColor=white)](https://ai.google.dev/)
[![Aoede Neural Voice](https://img.shields.io/badge/Aoede_Neural_TTS-Studio_Clarity-BE123C?style=flat-square&logo=podcast&logoColor=white)](https://ai.google.dev/)
...
[![Firebase Hosting](https://img.shields.io/badge/Firebase_Hosting-FFCA28?style=flat-square&logo=firebase&logoColor=black)](https://firebase.google.com/)
...
```mermaid
graph TD
    A["Student Client (Browser)"] --> B["Ziv Frontend (React 18 + Vite 6 + Tailwind v4)"]
    
    subgraph "Real-Time Voice Engine"
        B --> C["Web Speech API (STT / Indian English & Hindi)"]
        C --> D["Gemini 2.5 Flash (thinkingBudget: 0, Sub-400ms)"]
        D --> E["Gemini Aoede / Edge Neural TTS (Studio Voice)"]
        E --> F["Web Audio API (Gain Booster + Presence Filter + Spectrum Analyser)"]
        F --> G["Myraa Core Visualizer (Video States + Particles + Waveform)"]
    end

    subgraph "Coding & Execution Engine"
        B --> H["8,866+ DSA Problem Library"]
        H --> I["In-Browser Code Editor & Test Harness"]
        I --> J["Pyodide Wasm (Client Python Sandbox)"]
        I --> K["Piston REST API (Isolated Multi-Language Runtime)"]
    end

    subgraph "Intelligence & Memory"
        B --> L["Memory Brain (8-Layer Student Personalization Vault)"]
        B --> M["Groq LPU (Qwen 2.5 / LLaMA 3.3 120B / DeepSeek)"]
        B --> N["Firebase Firestore (Profiles, Progress & Analytics)"]
    end
```
